    ...
    static public void main(String[] args) {    
        ...
        for (int i = 0; i < args.length; i++) { 
            if ("inmemory".equals(args[i])) { 
                ...
                db.setProperty("perst.object.cache.init.size", Integer.valueOf(1013));
            } else if ("serializable".equals(args[i])) { 
                db.setProperty("perst.alternative.btree", Boolean.TRUE);
                serializableTransaction = true;
            } else if ("gc".equals(args[i])) { 
                db.setProperty("perst.gc.threshold", Integer.valueOf(1024*1024));
        ...
        //db.gc();
        System.out.println("=> "+ Benchmark.class.getName());
