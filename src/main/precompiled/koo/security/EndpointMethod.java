 ...
 *       public static final EndpointMethod GET_ACTOR = new GetActorEndpoint(AActor.class);
 *       public static final EndpointMethod DO_INTERNAL = new InternalCalcEndpoint(AActor.class, false);
 *   }
 */
public abstract class EndpointMethod {
    ...
    public boolean execute(JSONObject in, JSONObject out, Connection db, ProcessServlet servlet) {
        ...
        AActor caller = getCaller(servlet);
        if (caller != null) {
            // Agreement MUST exist (enforced by AActor constructor)
    ...
    /**
     * Helper to get authenticated AActor from servlet
     */
    private AActor getCaller(ProcessServlet servlet) {
