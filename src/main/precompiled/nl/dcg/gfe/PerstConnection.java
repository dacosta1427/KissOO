...
/**
 * PerstConnection - Wraps a kissweb Connection and adds Perst operations.
 * 
 * This class implements the standard Connection interface while adding
 * Perst-specific methods for OODBMS operations.
 * 
 * Usage in services:
 * 
 * <pre>
 * void myService(JSONObject injson, JSONObject outjson, Connection db, Servlet servlet) {
 *     // Standard kissweb operations
 *     Record rec = db.newRecord("users");
 *     
 *     // Perst operations (if enabled)
 *     if (db instanceof PerstConnection) {
 *         Actor actor = ((PerstConnection) db).retrieveObject(Actor.class, uuid);
 *     }
 * }
 * </pre>
 */
public class PerstConnection implements Connection {
    
    private final Connection delegate;
    private final PerstContext perstContext;
    
    /**
     * Create a PerstConnection wrapping an existing Connection
     * 
     * @param delegate The original kissweb Connection (PostgreSQL)
     */
    public PerstConnection(Connection delegate) {
        this.delegate = delegate;
        this.perstContext = PerstContext.getInstance();
    }
    ...
    public boolean isPerstAvailable() {
        return perstContext.isAvailable();
    }
    
    // ==================== Perst-Specific Methods ====================
    ...
    public <T extends CVersion> T retrieveObject(Class<T> clazz, String uuid) {
        checkPerstAvailable();
        return perstContext.retrieveObject(clazz, uuid);
    }
    ...
    public <T extends CVersion> T retrieveObject(Class<T> clazz, String field, String value) {
        checkPerstAvailable();
        return perstContext.retrieveObject(clazz, field, value);
    }
    
    /**
     * Retrieve all objects of a given type
     */
    public <T extends CVersion> java.util.Collection<T> retrieveAllObjects(Class<T> clazz) {
        checkPerstAvailable();
        return perstContext.retrieveAllObjects(clazz);
    }
    ...
    public void storeNewObject(CVersion obj) {
        checkPerstAvailable();
        perstContext.storeNewObject(obj);
    }
    ...
    public void storeModifiedObject(CVersion obj) {
        checkPerstAvailable();
        perstContext.storeModifiedObject(obj);
    }
    ...
    public void rollbackTransaction() {
        checkPerstAvailable();
        perstContext.rollbackTransaction();
    }
    
    private void checkPerstAvailable() {
        if (!isPerstAvailable()) {
            throw new IllegalStateException("Perst is not available. Check PerstEnabled in application.ini");
        }
    }
    
    // ==================== Delegate all java.sql.Connection methods ====================
