...
function Navbar($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const tt = (key) => t(key, void 0, store_get($$store_subs ??= {}, "$currentLocale", currentLocale));
    $$renderer2.push(`<header class="sticky top-0 z-50 bg-white shadow-sm border-b"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="flex justify-between items-center h-16"><div class="flex items-center"><button class="text-2xl font-bold text-gray-900 hover:text-gray-700">KissOO Svelte 5</button></div> <nav class="hidden md:flex items-center space-x-4"><div class="flex items-center mr-2"${attr("title", session.isAuthenticated ? "Logged in" : "Not logged in")}>`);
    ...
    $$renderer2.push(`<!--]--></div> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.home"))}</button> `);
    if (session.isAuthenticated) {
      $$renderer2.push("<!--[0-->");
      if (session.isAdmin) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.houses"))}</button> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.owners"))}</button> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.cleaners"))}</button> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.bookings"))}</button> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.schedules"))}</button> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.users"))}</button>`);
      } else if (session.ownerOid > 0) {
        $$renderer2.push("<!--[1-->");
        $$renderer2.push(`<button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.houses"))}</button> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.bookings"))}</button> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.schedules"))}</button>`);
      } else if (session.cleanerOid > 0) {
        $$renderer2.push("<!--[2-->");
        $$renderer2.push(`<button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("nav.schedules"))}</button>`);
      } else {
        $$renderer2.push("<!--[-1-->");
      }
      $$renderer2.push(`<!--]--> `);
      LanguageSwitcher($$renderer2);
      $$renderer2.push(`<!----> <button class="text-red-600 hover:text-red-800 font-medium">${escape_html(tt("nav.logout"))}</button>`);
    } else {
      $$renderer2.push("<!--[-1-->");
      LanguageSwitcher($$renderer2);
      $$renderer2.push(`<!----> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("auth.login_button"))}</button> <button class="text-gray-600 hover:text-gray-900 font-medium">${escape_html(tt("auth.signup_button"))}</button>`);
    }
