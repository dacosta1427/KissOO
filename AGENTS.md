...
11. **Don't ship the Svelte `node_modules` in the WAR**: `Tasks.java` `buildSystem()` does
 ...
 entire `koo/frontend/svelte` tree — including its multi-thousand-file `node_modules` (plus `.svelte-kit`/
 `build`/`dist`). That bloats the WAR/deploy and shows up as `node_modules` in the served tree. The SvelteKit
 app is a dev-only workspace (run via `npm run dev`, built separately) and is **not** read from `WEB-INF/precompiled`
 at runtime. After that `copyTree`, prune with `rmTree(.../svelte/node_modules)`, `rmTree(.../svelte/.svelte-kit)`, etc.
 Note `copyTree` has no exclude param, so the prune-after approach is used; if build time becomes an issue, switch the
 copy to exclude the `svelte` subdir entirely.
